...
function readEnv(env, name, fallback = "") {
  return String(env[name] || env[`APPSETTING_${name}`] || fallback).trim();
}

function resolveTunnelSettings(env = process.env) {
  const token = readEnv(env, "CLOUDFLARE_TUNNEL_TOKEN");
  const credentialsFile = readEnv(env, "CLOUDFLARE_TUNNEL_CREDENTIALS_FILE");
  ...
  const explicitAccountId = readEnv(env, "CLOUDFLARE_ACCOUNT_ID");
  const explicitTunnelId = readEnv(env, "CLOUDFLARE_TUNNEL_ID");
  const explicitCredentials =
    explicitAccountId && explicitTunnelId
      ? {
          AccountTag: explicitAccountId,
          TunnelID: explicitTunnelId,
          TunnelSecret: credentialsFromToken?.TunnelSecret || "",
        }
      : null;
  const credentials = explicitCredentials || credentialsFromFile || credentialsFromToken;
  const localConfigFlag = readEnv(env, "CLOUDFLARE_TUNNEL_LOCAL_CONFIG").toLowerCase();
  const remoteConfigFlag = readEnv(env, "CLOUDFLARE_TUNNEL_REMOTE_CONFIG").toLowerCase();
  const useLocalConfig = localConfigFlag === "true" || remoteConfigFlag === "false";
  ...
  return {
    ...
    credentials,
    credentialsFile,
    accountId: explicitAccountId || credentials?.AccountTag || "",
    tunnelId: explicitTunnelId || credentials?.TunnelID || "",
    nodeHostname: normalizeHostname(
      readEnv(env, "TUNNEL_NODE_HOSTNAME") || readEnv(env, "NODE_HOSTNAME")
    ),
    btHostname: normalizeHostname(
      readEnv(env, "TUNNEL_BT_HOSTNAME") || readEnv(env, "BT_HOSTNAME")
    ),
    xrayPort: parsePort(readEnv(env, "XRAY_PORT"), DEFAULT_XRAY_PORT),
    btPort: parsePort(readEnv(env, "BT_PORT"), DEFAULT_BT_PORT),
    portDomain: normalizeHostname(readEnv(env, "TUNNEL_PORT_DOMAIN")),
    portHostPrefix: readEnv(env, "TUNNEL_PORT_HOST_PREFIX", "p") || "p",
    portHostTemplate: readEnv(env, "TUNNEL_PORT_HOST_TEMPLATE"),
    apiToken: readEnv(env, "CLOUDFLARE_API_TOKEN"),
    useLocalConfig,
    useRemoteConfig: !useLocalConfig,
  };
}
...
function buildTunnelArtifacts(options = {}) {
  ...
  const configYaml = renderConfigYaml(settings.tunnelId, credentialsPath, ingressRules);
