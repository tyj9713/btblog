...
class CloudflareTunnelManager {
  ...
  isEnabled() {
    ...
    const token = String(
      config.CLOUDFLARE_TUNNEL_TOKEN ||
        this.env?.CLOUDFLARE_TUNNEL_TOKEN ||
        this.env?.APPSETTING_CLOUDFLARE_TUNNEL_TOKEN ||
        ""
    ).trim();
  ...
  async buildLogSummary(status = null) {
    ...
    const lines = [
      ...
      `pid: ${current.pid || this.readTunnelPid() || "-"}`,
      `processes: ${current.processes || (await this.readProcessList()) || "-"}`,
    ];
    if (this.lastError) {
  ...
  readTunnelPid() {
    const pidPath = path.join(this.cwd, "named-tunnel.pid");
  ...
  async status() {
    ...
    const processes = await this.readProcessList();
    return {
      enabled: this.isEnabled(),
      running,
      pid: this.readTunnelPid(),
      processes,
