...
const { runScript, resolveScript, shellQuote } = require("./script-runner");
...
class ServiceManager {
  ...
  async status() {
    const { stdout } = await runScript(this.execAsync, "process-status-suoha.sh", {
      cwd: this.cwd,
      timeout: 5000,
      maxBuffer: 1024 * 1024,
    });
  ...
  async killProcesses() {
    await runScript(this.execAsync, "stop-services.sh", {
      cwd: this.cwd,
      timeout: 10_000,
      env: {
        NAMED_TUNNEL_ENABLED: this.namedTunnelEnabled ? "true" : "false",
      },
    });
  }
  ...
  async _start(reason) {
    ...
    if (normalizeShellScriptLineEndings(resolvedScript)) {
      this.writeStartLog(`normalized CRLF line endings for ${resolvedScript}\n`);
    }
    this.writeStartLog(`\n[${new Date().toISOString()}] starting suoha service: ${reason}\nscript=${resolvedScript}\ncwd=${this.cwd || process.cwd()}\n`);

    let stdout = "";
    try {
      const result = await runScript(this.execAsync, path.basename(resolvedScript), {
        cwd: this.cwd,
