...
function parseProcessStatus(stdout) {
  const lines = processLines(stdout);
  const xrayRunning = lines.some((line) => /(^|\s)(\.\/)?xray\/xray(\s|$)/.test(line));
  const argoRunning = lines.some((line) => /(^|\s)(\.\/)?cloudflared-linux(\s|$)/.test(line));
...
class ServiceManager {
  ...
  async killProcesses() {
    ...
    await this.execAsync("pkill -9 -f 'cloudflared-linux' || true", {
      cwd: this.cwd,
      timeout: 10000,
    });
  }
